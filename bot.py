...
def start_message(message):
    bot.send_message(message.chat.id, "Привіт, {0.first_name}!👋".format(message.from_user, bot.get_me()))
...
def change_name(message):
    global msg
    msg = bot.send_message(message.chat.id, "Напиши мені своє ім'я та прізвище")
...
def verification(message):
    ...
    bot.send_message(message.chat.id, f"Тебе звати {message.text}")
    bot.send_message(message.chat.id, "Правильно?", reply_markup=markup_inline_yn)
